  ...
  <div class="pt-10 w-1/3 min-w-[480px] ml-auto mr-auto border">
    ...
    <form @submit.prevent="loginSubmit" class="pt-10">
      ...
      <div class="relative z-0 mb-6 mr-auto ml-auto w-3/4 group">
        ...
        <label v-if="!state.email_check" for="floating_email" class="absolute text-[4px] text-[#fe5358] dark:text-[#fe5358] -bottom-3.5 right-0">가입된 메일이 아니거나</label>
      </div>
      <!-- 비밀번호 입력 -->
      <div class="relative z-0 mb-3 mr-auto ml-auto w-3/4 group"> 
        ...
        <label v-if="!state.pw_check" for="floating_password" class="absolute text-[4px] text-[#fe5358] dark:text-[#fe5358] -bottom-3.5 right-0">비밀번호가 바르지 않습니다</label>
      </div>
