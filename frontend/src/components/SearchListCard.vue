<template>
 <div @mouseenter="cardToggle" @mouseleave="cardToggle" class="relative ml-auto mr-auto mb-10">
    ...
    <div class="w-[400px] h-[500px] z-0 border-2 rounded-lg border-[#2c5172] dark:border-neutral-500 bg-[#fffbf8] dark:bg-[#4c4c4c]">
      <img class="rounded-t-lg w-[400px] h-[225px]" :src="searchResultThumbnail" alt=""/>
      <div class="pt-6 pb-[11px] px-6 flex flex-col justify-between h-[275px] dark:text-neutral-100">
        <div>
          <div class="text-3xl text-justify font-bold dark:font-semibold h-[72px]">{{ searchResult.title }}</div>
          <div class="text-xl font-bold dark:font-semibold text-end">{{ searchResult.teacher }}</div>
        </div>
        <div class="flex flex-col">
          <div class="text-lg text-end" v-for="lecturetime in searchResult.lectureTime" :key="lecturetime">
            <p>{{lecturetime}}</p>
          </div>
        </div>
      </div>
    ...
    <div v-if="state.isMouseIn" class="absolute top-0 z-10 px-6 flex flex-col w-[400px] h-[500px] bg-black bg-opacity-80 rounded-lg border-2 border-black">
      <div class="h-[225px] flex justify-center items-center">
        <div class="text-3xl font-bold text-white">{{ searchResult.title }}</div>
      </div>
  ...
  <!-- 수강 신청 Modal -->
  <div class="modal fade bg-black bg-opacity-50 fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" :id="applySearchModalId" tabindex="-1" aria-labelledby="classinModalLabel" aria-hidden="true" @click.self="modalClose">
      ...
      <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white dark:bg-[#4c4c4c] bg-clip-padding rounded-md outline-none text-current">
        ...
        <div class="modal-body mt-4 pt-2 pb-4 px-10 text-center">
          ...
          <div class="flex flex-wrap w-full mb-6">
            ...
            <div class="w-[260px] mb-6 text-justify text-lg font-medium">{{ searchResult.title }}</div>
            <div class="w-[80px] mr-[20px] mb-6 text-start text-lg font-medium border-r border-neutral-400 dark:border-neutral-500">강의자</div>
            <div class="w-[260px] mb-6 text-justify text-lg font-medium">{{ searchResult.teacher }}</div>
            ...
            <div class="w-[260px] mb-6 text-justify text-lg font-medium">{{ searchResult.startDate }} ~ {{ searchResult.endDate }}</div>
            ...
            <div class="w-[260px] mb-6 flex flex-col">
              <div class="text-start text-lg font-medium" v-for="lecturetime in searchResult.lectureTime" :key="lecturetime">
                <p>{{lecturetime}}</p>
              </div>
            </div>
            ...
            <div class="w-[260px] mb-6 text-justify text-lg font-medium">{{ searchResult.content }}</div>
          </div>
...
const searchResultThumbnail = store.state.lecture.lectureImgFirstUrl+props.searchResult.lectureImg

const applySearchModalId = 'applySearch'+props.searchResult.lectureId
...
const modalOpen = () => {
  document.getElementById(applySearchModalId).classList.replace('hidden', 'show')
}

const modalClose = () => {
  document.getElementById(applySearchModalId).classList.replace('show', 'hidden')
}
...
const registration = async () => {
  await axios({
    ...
    data: {
      ...
      lectureId : props.searchResult.lectureId,
    }
  })
  .then(res => {
