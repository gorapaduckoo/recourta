<template>
	...
	<div class="flex justify-center mb-3 w-[450px] sm:w-[600px] h-[170px] ml-auto mr-auto">
		<div class="flex flex-col border w-full rounded-lg justify-between p-6 odd:bg-[#fffbf8]">
			<div class="text-gray-900 text-2xl font-bold mb-2">{{ searchResult.title }}</div>
			<div class="flex justify-between mt-5">
				<div class="text-xl font-semibold text-end">{{ searchResult.teacher }}</div>
				<div class="space-x-5">
					<button class="py-1.5 px-3 text-white rounded-md bg-[#2c5172]" @click="modalOpen">수강신청</button>
				</div>
			</div>
		</div>
	</div>
	...
	<!-- 수강 신청 Modal -->
  <div class="modal fade bg-black bg-opacity-50 fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" :id="applyModalId" tabindex="-1" aria-labelledby="mClassinModalLabel" aria-hidden="true" @click.self="modalClose">
...
const applyModalId = 'mApply'+props.searchResult.lectureId
